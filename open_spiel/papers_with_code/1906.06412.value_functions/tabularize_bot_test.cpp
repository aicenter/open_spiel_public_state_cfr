...
namespace open_spiel {
    namespace papers_with_code {
        namespace {
            void ComparePolicySize(const std::shared_ptr<TabularPolicy>& joint_policy, const std::shared_ptr<const Game>& game) {
                TabularPolicy full_policy = GetUniformPolicy(*game);

                SPIEL_CHECK_EQ(full_policy.PolicyTable().size(), joint_policy->PolicyTable().size());

                for(const auto& info_state_policy : full_policy.PolicyTable()) {
                    SPIEL_CHECK_EQ(info_state_policy.second.size(),
                            joint_policy->GetStatePolicy(info_state_policy.first).size());
                }
            }

            void ComparePolicyToOnlinePlay(const std::shared_ptr<TabularPolicy>& joint_policy,
                                           BotParameters params, const std::shared_ptr<const Game>& game, int num_games) {
                ...
                for (int i = 0; i < num_games; i++) {
                    ...
                    for (int player = 0; player < 2; player++) {
                        ...
                        while (!state->IsTerminal()) {
                            std::pair<ActionsAndProbs, Action> step = bots[player]->StepWithPolicy(*state);
                            ...
                            if(state->IsPlayerActing(player)) {
                                ...
                                std::vector<double> offline_strategy = GetProbs(
                                        joint_policy->GetStatePolicy(state->InformationStateString(player)));
                                for (int action_index = 0; action_index < online_strategy.size(); action_index++) {
                                    SPIEL_CHECK_FLOAT_EQ(online_strategy[action_index], offline_strategy[action_index]);
                                }
                            }
            ...
            CreatePolicyFromSetup(const BotParameters& params, const std::shared_ptr<const Game>& game) {
                ...
                std::unique_ptr<SherlockBot> bot_player_two = bot_factory.Create(game, Player(1),
                                                                                 params, true);
                std::shared_ptr<TabularPolicy> joint_policy = tabularize_bot::FullBotPolicy(
