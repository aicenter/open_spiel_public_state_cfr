...
std::string BettingAbstractionToString(const BettingAbstraction &betting) {
  switch (betting) {
    ...
    default:SpielFatalError("Unknown betting abstraction.");
      break;
  }
}
...
class UniversalPokerObserver : public Observer {
 public:
  UniversalPokerObserver(IIGObservationType iig_obs_type)
  ...
  static void WriteObservingPlayer(const UniversalPokerState &state, int player,
                                   Allocator *allocator) {
    auto out = allocator->Get("player", {state.NumPlayers()});
    out.at(player) = 1;
  }

  // Private card of the observing player. One-hot vector of size num_cards.
  static void WriteSinglePlayerCard(const UniversalPokerState &state, int player,
                                    Allocator *allocator) {
    std::cout << state.acpc_game_->NumSuitsDeck() << "\n";
    std::cout << state.acpc_game_->NumRanksDeck() << "\n";
    const logic::CardSet full_deck(state.acpc_game_->NumSuitsDeck(),
                                   state.acpc_game_->NumRanksDeck());
    auto out = allocator->Get("private_card", {full_deck.NumCards()});
    const std::vector<uint8_t> deckCards = full_deck.ToCardArray();
    logic::CardSet holeCards = state.HoleCards(player);
    for (uint32_t i = 0; i < full_deck.NumCards(); i++) {
      if (holeCards.ContainsCards(deckCards[i])) {
        out.at(i) = 1;
      }
    }
  }
  ...
  static void WriteAllPlayerCards(const UniversalPokerState &state,
                                  Allocator *allocator) {
    const logic::CardSet full_deck(state.acpc_game_->NumSuitsDeck(),
                                   state.acpc_game_->NumRanksDeck());
    auto out = allocator->Get("private_cards",
                              {state.num_players_, full_deck.NumCards()});
    const std::vector<uint8_t> deckCards = full_deck.ToCardArray();
    for (int p = 0; p < state.NumPlayers(); p++) {
      logic::CardSet holeCards = state.HoleCards(p);
      for (uint32_t i = 0; i < full_deck.NumCards(); i++) {
        if (holeCards.ContainsCards(deckCards[i])) {
          out.at(p, i) = 1;
        }
  ...
  static void WriteCommunityCards(const UniversalPokerState &state,
                                  Allocator *allocator) {
    const logic::CardSet full_deck(state.acpc_game_->NumSuitsDeck(),
                                   state.acpc_game_->NumRanksDeck());
    const std::vector<uint8_t> deckCards = full_deck.ToCardArray();
  ...
  static void WriteDealingInitialCard(const UniversalPokerState &state,
                                      Allocator *allocator) {
  ...
  static void WriteBettingSequence(const UniversalPokerState &state,
                                   Allocator *allocator) {
    const std::string actionSeq = state.GetActionSequence();
    const int length = actionSeq.length();
    SPIEL_CHECK_LT(length, state.game_->MaxGameLength());
    auto out = allocator->Get("betting", {state.game_->MaxGameLength(), 2});
  ...
  static void WritePotContribution(const UniversalPokerState &state,
                                   Allocator *allocator) {
    auto out = allocator->Get("pot_contribution", {state.NumPlayers()});
    for (auto p = Player{0}; p < state.NumPlayers(); p++) {
      out.at(p) = state.acpc_state_.Ante(p);
    }
  }
...
//  // observation into.
  void WriteTensor(const State &observed_state, int player,
    ...
    auto &state = open_spiel::down_cast<const UniversalPokerState &>(observed_state);
  ...
  std::string StringFrom(const State &observed_state,
    ...
    auto acpc_game_ = state.acpc_game_;
    auto acpc_state_ = &state.acpc_state_;
    if (iig_obs_type_.private_info == PrivateInfoType::kSinglePlayer) {
      absl::StrAppend(&result, "[Observer: ", player, "]");
      absl::StrAppend(&result, "[Private: ", state.HoleCards(player).ToString(), "]");
    } else if (iig_obs_type_.private_info == PrivateInfoType::kAllPlayers) {
      absl::StrAppend(&result, "[Privates:");
      for (Player iter_player = 0; player < state.NumPlayers(); iter_player++) {
        absl::StrAppend(&result, " ", state.HoleCards(iter_player).ToString());
      }
    ...
    if (iig_obs_type_.public_info) {
      ...
      absl::StrAppend(&result, "[Money: ", money, "]");
      absl::StrAppend(&result, "[Public: ", state.BoardCards().ToString(), "]");
      if (iig_obs_type_.perfect_recall) {
        ...
        absl::StrAppend(&result, "[Sequences: ", sequences, "]");
      } else {
        // Pot contributions (imperfect recall)
        absl::StrAppend(&result, "[Ante: ");
        for (Player iter_player = 0; iter_player < state.NumPlayers(); iter_player++) {
          absl::StrAppend(&result, " ", acpc_state_->Ante(iter_player));
        }
        absl::StrAppend(&result, "]");
      }
    }
...
void UniversalPokerState::InformationStateTensor(
  ...
  const UniversalPokerGame &game = open_spiel::down_cast<const UniversalPokerGame &>(*game_);
...
UniversalPokerGame::UniversalPokerGame(const GameParameters &params)
  ...
  default_observer_ = std::make_shared<UniversalPokerObserver>(kDefaultObsType);
  info_state_observer_ = std::make_shared<UniversalPokerObserver>(kInfoStateObsType);
...
std::vector<int> UniversalPokerGame::InformationStateTensorShape() const {
  ...
  return {2 * num_players + 2 * total_num_cards + 2 * gameLength};
}
...
int UniversalPokerGame::MaxGameLength() const {
  ...
  // Bet Actions
  if (acpc_game_.IsLimitGame()) {
    // In limit game the stack is int.max so we use different computation to avoid game length in thousands.
    const auto &acpc_game = acpc_game_.Game();
    for (int raises : acpc_game.maxRaises) {
      length = length + raises;
    }
  } else {
    double maxStack = 0;
    double maxBlind = 0;
...
open_spiel::Action ACPCActionToOpenSpielAction(
  ...
  switch (action.type) {
    case project_acpc_server::ActionType::a_fold:return ActionType::kFold;
    case project_acpc_server::ActionType::a_call:return ActionType::kCall;
    case project_acpc_server::ActionType::a_raise:SPIEL_CHECK_NE(state.betting(), BettingAbstraction::kFC);
    ...
    case project_acpc_server::ActionType::a_invalid:SpielFatalError("Invalid action type.");
    default:SpielFatalError(absl::StrCat("Type not found. Type: ", action.type));
  }
...
std::shared_ptr<Observer> UniversalPokerGame::MakeObserver(
    absl::optional<IIGObservationType> iig_obs_type,
    const GameParameters &params) const {
  if (params.empty()) {
    return std::make_shared<UniversalPokerObserver>(
